...
void afficher_alarme(GtkWidget  *liste)
{
	...
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(" CapJour",renderer, "text", ECapJour,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("CapHeure",renderer, "text",ECapHeure,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);


	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("CapMois",renderer, "text",ECapMois,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("CapEtat",renderer, "text",ECapEtat,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);
	...
	{
		f= fopen("fumee.txt","a+");
	 	while(fscanf(f,"%d %d %d %d \n",&CapJour,&CapHeure,&CapMois,&CapEtat)!=EOF)	          						  
		{ if(CapEtat==1){
			gtk_list_store_append(store, &iter);
			gtk_list_store_set(store,&iter, ECapJour,CapJour,
			ECapHeure,CapHeure,ECapMois,CapMois,ECapEtat,CapEtat,-1);
		}}
		fclose(f);
		gtk_tree_view_set_model (GTK_TREE_VIEW(liste), GTK_TREE_MODEL(store));
		g_object_unref (store);
...
void afficher_alarme1(GtkWidget  *liste)
{
	...
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(" CapJour",renderer, "text", FCapJour,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("CapHeure",renderer, "text",FCapHeure,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);


	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("CapMois",renderer, "text",FCapMois,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);

	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("CapEtat",renderer, "text",FCapEtat,NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (liste),column);
	...
	{
		g= fopen("mouvement.txt","a+");
	 	while(fscanf(g,"%d %d %d %d \n",&CapJour,&CapHeure,&CapMois,&CapEtat)!=EOF)	          						  
		{ if((CapEtat==1)&&(6>CapHeure)&&(CapHeure<24)){
			gtk_list_store_append(store, &iter);
			gtk_list_store_set(store,&iter, FCapJour,CapJour,
			FCapHeure,CapHeure,FCapMois,CapMois,FCapEtat,CapEtat,-1);
		}}
		fclose(g);
		gtk_tree_view_set_model (GTK_TREE_VIEW(liste), GTK_TREE_MODEL(store));
		g_object_unref (store);
